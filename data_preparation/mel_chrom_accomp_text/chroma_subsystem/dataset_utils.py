...
models_path = '/media/maindisk/maximos/data/pretrained_models/'
...
class LiveMelCATDataset(Dataset):
    def __init__(self, midis_folder, segment_size=64, resolution=24, max_seq_len=1024, only_beginning=False,\
                 ignore_short_pieces=True, gct_tokenization=True):
    ...
    def __getitem__(self, idx):
        ...
        if main_piece.tracks is not None:
            main_piece_size = main_piece.tracks[0].pianoroll.shape[0]
        else:
            # load previous
            print('track was None, loading previous:', self.midis_list[idx])
            main_piece = pypianoroll.read(self.midis_folder + os.sep + self.midis_list[idx-1], resolution=self.resolution)
            main_piece_size = main_piece.tracks[0].pianoroll.shape[0]
        # check if piece is long enough
        if self.ignore_short_pieces:
            while main_piece_size <= self.segment_size*main_piece.resolution:
        ...
        # remove drums tracks
        mpu.remove_drum_tracks_in_place( new_piece )

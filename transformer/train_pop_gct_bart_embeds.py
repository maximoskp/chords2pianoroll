...
model = MelCAT_GCT(bart_config, gpu=0)
# dev = torch.device("cpu")
# model = MelCAT_base(bart_config, gpu=None)

if load_saved:
    checkpoint = torch.load('saved_models/bart_gct_pop_embeds/bart_gct_pop_embeds.pt', weights_only=True)
...
dataset = LiveMelCATDataset(midifolder, segment_size=40, resolution=4, \
    max_seq_len=1024, only_beginning=True, gct_tokenization=True)
